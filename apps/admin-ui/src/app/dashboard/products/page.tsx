...
const ProductList = () => {
    ...
    const {data, isLoading} : UseQueryResult<any> = useQuery({
        queryKey:['all-products', page, deferredFilter],
        queryFn:async () => {
            const res=await axiosInstance.get(`/admin/api/get-all-products?page=${page}&limit=${limit}&search=${deferredFilter}`);
            return res.data;
        },
    ...
    const totalProducts = data?.meta.totalProducts ?? 0;
        ...
        () =>[
            ...
            },
            {
                ...
                <div className='flex items-center gap-1 text-yellow-400'>
                    <Star fill="#FFD700" size={18} className="text-yellow-400" />
                    <span className="text-white font-medium">{row.original.rating ? row.original.rating.toFixed(1) : 'N/A'}</span>
                </div>
            ),
            },
            {
    ...
    const exportCSV = () => {
        const headers = ['ID', 'Title', 'Shop', 'Price', 'Stock', 'Category', 'Rating'];
        const csvData = allProducts.map((product: any) =>
            [
                `"${product.id}"`,
                `"${String(product.title || '').replace(/"/g, '""')}"`,
                `"${String(product.Shop?.name || 'N/A').replace(/"/g, '""')}"`,
                product.sale_price,
                product.stock,
        ...
        link.href = url;
        link.setAttribute('download', `products-page-${page}-${new Date().getTime()}.csv`);
                        ...
                        {totalPages > 1 && (
                            <div className="flex items-center justify-between px-6 py-4 bg-gray-800 border-t border-gray-700">
                                <div className="text-sm text-gray-400">
                                    ...
                                    <span className="font-medium text-white">{totalProducts}</span> results
                                </div>
