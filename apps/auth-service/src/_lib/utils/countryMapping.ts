 ...
 * Converts between ISO 3166-1 numeric and alpha-2 codes
 */

// Mapping from numeric (from countries-110m.json) to alpha-2 (for Stripe)
export const NUMERIC_TO_ALPHA2: Record<string, string> = {
    ...
    '458': 'MY', '462': 'MV', '466': 'ML', '470': 'MT', '474': 'MH',
    '478': 'MR', '480': 'MU', '484': 'MX', '492': 'MC', '496': 'MN',
    '498': 'MD', '499': 'ME', '504': 'MA', '508': 'MZ', '512': 'OM',
    '516': 'NA', '520': 'NR', '524': 'NP', '528': 'NL', '540': 'NC',
...
/**
 * Convert ISO numeric code to alpha-2 code
 * @param numericCode - ISO 3166-1 numeric code (e.g., '840')
 * @returns ISO alpha-2 code (e.g., 'US') or fallback to 'US'
 */
export const numericToAlpha2 = (numericCode: string | null | undefined): string => {
    if (!numericCode) return 'US';
    
    const alpha2 = NUMERIC_TO_ALPHA2[numericCode];
    if (alpha2) return alpha2;
    
    // If it's already a 2-character alpha code, return as-is
    if (numericCode.length === 2 && /^[A-Z]{2}$/i.test(numericCode)) {
        return numericCode.toUpperCase();
    }
    
    // Fallback to US
    console.warn(`Unknown country code: ${numericCode}, defaulting to US`);
    return 'US';
};
