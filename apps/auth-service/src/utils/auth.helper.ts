...
export const checkOtpRestrictions = async (email: string, next: NextFunction) => {
    ...
    if (await redis.get(`otp_spam_lock:${email}`)) {
        throw new ValidationError("Too many OTP requests! Try again after 1 hours.");
    }
    if (await redis.get(`otp_cooldown:${email}`)) {
        throw new ValidationError("OTP recently sent! Please wait 1 minutes before requesting again.");
    }
}
